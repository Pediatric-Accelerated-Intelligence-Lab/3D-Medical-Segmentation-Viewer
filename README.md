...
**Step by Step Example for Integrating MEN-RT container (accepts one imaging modality)**

1. Register the task name inside "class TaskName"
```
class TaskName(StrEnum):
      gli = "Pre- and Post-Treatment Adult Glioma"
      menrt = "Pre-Radiotherapy Meningioma (BraTS-MEN-RT)"
```
2. Add the docker image to DOCKER_TASK_DICT
```
DOCKER_TASK_DICT = {
    TaskName.gli.value:    ("aparida12/brats2025:gli",            "/input/", "ro"),
...
```
4. Add the display name to TASK_NAME_MAPPING
```
TASK_NAME_MAPPING = {
    TaskName.gli.value: "Adult Glioma Segmentation",
...
```
5. Add the required regions to RegionName and LABEL_MAPPING_FACTORY.
...
```
6. Indicate the scan types needed. This container requires only one.
   
```
TASK_MODALITIES[TaskName.menrt.value] = ["post-contrast T1-weighted"]
```
7. In DUMMY_FILE_NAMES, indicate what the input filenames look like.
```
DUMMY_FILE_NAMES = {
    TaskName.gli.value:   ALL_TASKS,
...
```
**Step by Step Example for Integrating PEDS container (accepts all 4 imaging modalities)**
...
6. Indicate the scan types needed. This container requires all four, so it is added to FULL_MODALITY_TASKS
